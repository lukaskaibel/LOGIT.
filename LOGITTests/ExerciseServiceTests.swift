        ...
        exerciseService.matchExercisesToExisting(exerciseNames)
            .sink(receiveCompletion: { completion in
                ...
                case .failure(let error):
                    XCTFail("ExerciseServiceTests: testMatchingExerciseToExistingExercises failed: \(error.localizedDescription)")
                }
        ...
        exerciseService.createExercise(for: "Bulgarian Split Squats")
            .sink(receiveCompletion: { completion in
                ...
                case .finished:
                    ...
                    break
                case .failure(let error):
                    XCTFail("ExerciseServiceTests: testCreateExerciseForName failed: \(error.localizedDescription)")
                }
                expectation.fulfill() // Fulfill the expectation when completed
