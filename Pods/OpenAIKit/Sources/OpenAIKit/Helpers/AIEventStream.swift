...
public final class AIEventStream<ResponseType: Decodable>: NSObject, URLSessionDataDelegate {
    ...
    private var onCompleteCompletion:
        ((_ code: Int?, _ forceEnd: Bool, _ error: Error?) throws -> Void)?
    ...
    public func urlSession(
        ...
        if let response = (dataTask.response as? HTTPURLResponse),
            let decodedError = try? JSONSerialization.jsonObject(with: data) as? [String: Any]
        {
...
extension AIEventStream {
    public func onStart(_ onStartCompletion: @escaping (() -> Void)) {
        self.onStartCompletion = onStartCompletion
    }

    public func onComplete(
        _ onCompleteCompletion: @escaping (
            (_ code: Int?, _ forceEnd: Bool, _ error: Error?) throws -> Void
        )
    ) {
    ...
    public func onMessage(
        _ onMessageCompletion: @escaping ((_ data: Data, _ message: ResponseType?) -> Void)
    ) {
