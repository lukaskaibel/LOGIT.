...
public final class OpenAIKitNetwork {
    ...
    func request<ResponseType: Decodable>(
        _ method: OpenAIHTTPMethod,
        url: String,
        body: Data? = nil,
        headers: OpenAIHeaders? = nil,
        ...
        let task = session.dataTask(with: request) { data, response, error in
            ...
            guard 200...299 ~= response.statusCode else {
                ...
                if let decodedError = try? JSONSerialization.jsonObject(with: data)
                    as? [String: Any]
                {
    ...
    func requestStream<ResponseType: Decodable>(
        _ method: OpenAIHTTPMethod,
        url: String,
        body: Data? = nil,
        headers: OpenAIHeaders? = nil,
        completion: @escaping (Result<AIStreamResponse<ResponseType>, Error>) -> Void
    ) {
        guard let url = URL(string: url) else {
            completion(.failure(OpenAINetworkError.invalidURL))
            return
            ...
            completion(
                .success(
                    AIStreamResponse(
                        ...
                        message: message,
                        data: data,
                        isFinished: streamState.isStreamFinished,
                        forceEnd: streamState.isStreamForceStop
                    )
                )
            )
        }
    ...
    func requestStream<ResponseType: Decodable>(
        ...
        body: Data? = nil,
        headers: OpenAIHeaders? = nil
    ) async throws -> AsyncThrowingStream<AIStreamResponse<ResponseType>, Error> {
        guard let url = URL(string: url) else {
        ...
        return AsyncThrowingStream<AIStreamResponse<ResponseType>, Error> { continuation in
            Task(priority: .userInitiated) {
                ...
                stream.onMessage { data, message in
                    continuation.yield(
                        AIStreamResponse(stream: stream, message: message, data: data)
                    )
                }
