...
extension OpenAIKit {
    ...
    public func sendCompletion(
        ...
        let requestBody = CompletionsRequest(
            model: model,
            prompt: prompt,
...
extension OpenAIKit {
    ...
    ///   - stop: Up to 4 sequences where the API will stop generating further tokens. The returned text will not contain the stop sequence.
    ///   - user: A unique identifier representing your end-user, which can help OpenAI to monitor and detect abuse.
    public func sendStreamCompletion(
        prompt: String,
        model: AIModelType,
        maxTokens: Int?,
        temperature: Double = 1,
        ...
        network.requestStream(
            endpoint.method,
            url: endpoint.urlPath,
            body: requestData,
            headers: headers
        ) { (result: Result<AIStreamResponse<AIResponseModel>, Error>) in
            completion(result)
        }
    }
    ...
    public func sendStreamCompletion(
        ...
        return try await network.requestStream(
            endpoint.method,
            url: endpoint.urlPath,
            body: requestData,
            headers: headers
