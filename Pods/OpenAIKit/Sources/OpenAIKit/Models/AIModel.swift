...
extension AIModelType {
    ...
    /// GPT-3.5 models can understand and generate natural language or code. Our most capable and cost effective model is gpt-3.5-turbo which is optimized for chat but works well for traditional completions tasks as well.
    public enum GPTv3_5Model: String {
...
public struct AIResponseModel: Codable {
    public struct Choice: Codable {
        ...
        public init(from decoder: Decoder) throws {
            let container: KeyedDecodingContainer<AIResponseModel.Choice.CodingKeys> =
                try decoder.container(keyedBy: AIResponseModel.Choice.CodingKeys.self)
            self.text = try container.decodeIfPresent(
                String.self,
                forKey: AIResponseModel.Choice.CodingKeys.text
            )
            self.message = try container.decodeIfPresent(
                AIMessage.self,
                forKey: AIResponseModel.Choice.CodingKeys.message
            )
            self.index = try container.decode(
                Int.self,
                forKey: AIResponseModel.Choice.CodingKeys.index
            )
            self.logprobs = try container.decodeIfPresent(
                Int.self,
                forKey: AIResponseModel.Choice.CodingKeys.logprobs
            )
            self.finishReason = try container.decodeIfPresent(
                String.self,
                forKey: AIResponseModel.Choice.CodingKeys.finishReason
            )

            if let deltaMessage = try? container.decodeIfPresent(
                AIMessageDelta.self,
                forKey: AIResponseModel.Choice.CodingKeys.delta
            ) {
                self.message = AIMessage(
                    role: deltaMessage.role ?? .assistant,
