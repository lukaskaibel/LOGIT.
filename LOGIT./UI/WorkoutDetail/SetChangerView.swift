...
struct SetChangerView: View {
    ...
    var body: some View {
        List {
            Section(content: {
                Picker("Repetitions", selection: $workoutSet.repetitions, content: {
            ...
            Section(content: {
                Picker("Weight", selection: $workoutSet.weight, content: {
                    ForEach(0..<10000, id:\.self) { i in
                        Text(String(i))
                    }
                }).pickerStyle(WheelPickerStyle())
            ...
            Section(content: {
                Picker("time", selection: $workoutSet.time, content: {
                    ForEach(0..<10000, id:\.self) { i in
                        Text(String(i))
                    }
                }).pickerStyle(WheelPickerStyle())
